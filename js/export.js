...
function statTitle(s) {
  const c = contents.find(x => x.id == s.contentId || x.id == Number(s.contentId) || (x.platform === s.platform && x.createdAt === (s.date || '')));
  return c ? c.title : (s.title || '');
}
...
function importData(event) {
  ...
  reader.onload = function(e) {
    try {
      ...
      if (data.reviews)  { reviews = data.reviews;       saveData('reviews', reviews);       picked.push('复盘' + reviews.length); }
      if (data.accountStats) { accountStats = data.accountStats; saveData('accountStats', accountStats); picked.push('账号数据' + accountStats.length); }
      if (data.accountIds)   { accountIds = data.accountIds;     saveData('accountIds', accountIds);     picked.push('账号ID' + accountIds.length); }
      render();
